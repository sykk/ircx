...
**The frontend in headless Chrome**, via `.Codex/skills/run-ircx/driver.mjs`,
...
`.Codex/skills/run-ircx/window.mjs`, which starts `Xvfb`, seeds a profile,
...
printf 'goto /\nss /tmp/shots/shell.png\ntext body\nquit\n' \
  | node .Codex/skills/run-ircx/driver.mjs
```
...
quit
' | node .Codex/skills/run-ircx/driver.mjs --seeded
```
...
quit
' | node .Codex/skills/run-ircx/window.mjs --join '#harness'
```
...
```bash
node .Codex/skills/run-ircx/startup.mjs --messages 100000 --networks 3 --runs 3
```
...
- **Do not type a path into GTK's *folder* chooser.** Its location bar (`ctrl+l`)
  ...
  `/tmp/nootaplugin`, and a longer one as `Codex-aude-1000` for `Codex-1000`.
...
- **Backend-dependent UI is empty, not broken.** Without `--seeded` every
  ...
  `npm run tauri dev`.
- **`.Codex/**` is already excluded** from vitest (`vite.config.ts`) and passes
...
- **Blank screenshot, no error**: the Tauri stub did not load. Check the driver
  was started with `--config .Codex/skills/run-ircx/vite.browser.config.mjs`
