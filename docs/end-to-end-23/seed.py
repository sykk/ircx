"""Fills a channel whose rows can change height after they are drawn.
...
So this channel groups. Three people rather than two, speaking in runs of up to
four lines, and two things that open a group:

  - A declared topic every 40th line, its name drawn from three that recur.
...
def speaker(n):
    """Runs of one to four lines, so blocks form and a regrouping can split
    one. Deterministic: the walk has to be reproducible from the seed alone."""
    return NICKS[(n // 3 + n // 11) % len(NICKS)]
...
for n in range(1, TOTAL + 1):
    ...
    if n % 40 == 0:
        text = f"[{TOPICS[(n // 40) % len(TOPICS)]}] {text}"
