"""The instrument for run 37: the window and the sockets, on a server that
tells the truth.

Run 36's shape with the proxy taken out — this run asks what the client draws
over its own rows, so nothing about the server needs to be a lie.  One process
holds the window and every socket, driven from a FIFO, so the order of a line
and the photograph after it is stated rather than hoped for.

    walk.py <fifo> <log> <shots> <worktree> <nick> <channel> [opts...]
...
fifo, logpath, shots, worktree, nick, channel = sys.argv[1:7]
...
win = subprocess.Popen(
    ["node", WINDOW, "--server", "127.0.0.1:6677",
     "--nick", nick, "--join", channel, "--keep"] + sys.argv[7:],
